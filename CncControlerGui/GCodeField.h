...
class GCodeField 
{
	...
	public:
		...
		////////////////////////////////////////////////////////
		explicit GCodeField(const GCodeField& field) 
		{
			setParmeters(field.getCmd(), field.getNum(), field.getSubNum());
		}

		GCodeField(char c, long n, long sn = INVALID_GCODE_FIELD_NUM )
		{
			setParmeters(c, n, sn);
		}
		
		explicit GCodeField(const char c)
		{
			const wxString tt(c);
			setToken(tt);
		}
		
		explicit GCodeField(const char* t)
		{
			if ( t != NULL )
			{
				const wxString tt(t);
		...
		////////////////////////////////////////////////////////
		explicit GCodeField(wxString& t) 
		{
			setToken(t);
		}
